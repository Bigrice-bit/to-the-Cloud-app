	...
	<view>
		...
		<view>
			<u-cell-group v-for="(i, index) in UnsignStu" :key="index">
				<u-cell-item :title="i.stuName" :label="i.stuNum" :arrow="false" @click="ChangeStatus(index)">
		...
		<view>
			...
			<u-cell-group v-for="(i, index) in SignStu" :key="index">
				<u-cell-item :title="i.stuName" :label="i.stuNum" :arrow="false">
					<u-icon slot="icon" size="100" name="../../../static/headimage.png"></u-icon>
	...
	export default {
		...
		onLoad(option) {
			...
			this.$Api.SignInfo(this.startSignId).then(async (res) => {
				...
				// /*  */console.log(res)
				let useobj = res.data.data
				if (res.data.success) {
					console.log(res.data.data)
					for (var i = 0; i < res.data.data.length; i++) {
						if (useobj[i].signStatus == '已签到') {
							_this.SignStu.push(useobj[i])
						} else {
							_this.UnsignStu.push(useobj[i])
						}
					}
				}
			})
				...
				uni.showActionSheet({
						...
						success: function(res) {
							...
							let data = {
								...
								StuId: _this.UnsignStu[index].stuId,
							...
							_this.$Api.UpdateStuStatus(data).then(res => {
									...
									if (res.data.success) {
										uni.showToast({
											...
											setTimeout(function() {
												uni.redirectTo({
													url: '/pages/class/SignIn/ManualReg?item='+ _this.startSignId
												})
											}, 1000);
											}
										})
